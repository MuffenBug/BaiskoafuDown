...
banner = f'''\033[01;36m\
 ...
 Downloader for Baiskoafu | MuffenBug | {VERSION} \033[1;33m\033[0;0m
  Thanks to r00t173 & proxygod [1;33m\033[0;0m
'''
print(banner)
...
def is_connected():
    try:
        # Attempt to connect to a common DNS server (Google's public DNS)
        ...
        def down_chunk(i):
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    response = requests.get(ts_url, stream=True, verify=False, timeout=60, headers=headers)
                    ...
                    if os.path.exists(ts_path) and os.path.getsize(ts_path) > 0:
                        ...
                        raise Exception("File was not written properly")
                    ...
                    if attempt < max_retries - 1:
                        print(f"Chunk {i} failed (attempt {attempt + 1}/{max_retries}), retrying in {retry_delay}s...")
                        time.sleep(retry_delay)
                        retry_delay *= 2  # Exponential backoff
                    else:
                        print(f"Chunk {i} failed after {max_retries} attempts: {e}")
                finally:
                    if response:
                        response.close()
...
def combine(file_name):
    ...
    # Check if output file is MP3 and convert accordingly
    if file_name.lower().endswith('.mp3'):
        ...
        # For video files (MP4, etc.), maintain quality with better encoding parameters
        ffmpeg_command = [
            'ffmpeg', '-y', '-f', 'concat', '-safe', '0', '-i', list_file_path,
